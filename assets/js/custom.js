	...
	$('#bidang').change(function(e) {
			...
			var id_jenis = $('#bidang').val();
			var dak_bku = $('#dak_bku').val();
		

			$.getJSON(root+'index.php/sub_jenis/json_by_id_jenis/'+dak_bku+'/'+ id_jenis, function (data) {
					// console.log(data);
					$('#sub_jenis').empty();
					$('#sub_jenis').append('<option value="">-- Pilih Sub Bidang --</option>');
					$.each(data.data, function (i, item) {
						$('#sub_jenis').append('<option value="'+item.id+'">'+item.sub_jenis+'</option>')
					})
					$('#sub_jenis').select2().select2("val", null);
				});
			return false;
		});
